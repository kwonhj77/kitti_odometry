...
def test(dataloaders: list[DataLoader], model: nn.Module, loss_fn: nn.Module, device: torch.device):
    ...
    test_recorder = TestResultRecorder(train=False)
    
    for idx, dataloader in enumerate(dataloaders):
        ds_recorder = DatasetResultRecorder(size=len(dataloader.dataset), num_batches=len(dataloader), dataset_idx=idx, train=False)
